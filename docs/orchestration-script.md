...
- 编辑器支持自动补全：
  - **JS**：完整 IntelliSense（Monaco 内置 TS 语言服务）——内置方法（`split`/`Math.*`/数组方法等）带签名与悬停；`devices.`、`poses.`、`utils.`、`log.`、`math.`（mathjs 成员，服务端实时探测）、`require('模块')` 后列出该模块导出成员。
  - **Python**：内置函数/类型（`enumerate(`、`len(` 等）+ 关键字 + 常用类型方法（`s.split(` 场景）；`import math` 后 `math.` 列出标准库成员（含 docstring 签名），覆盖 math/os/json/time/random/re/collections 等常用 stdlib；离线/mock 模式回退内置补全表，功能不减。
  - 补全数据源：服务端用与运行一致的 Python 解释器生成标准库快照（localStorage 缓存 7 天），失败自动回退内置表。
  - **Python 实时语法检查**：py 文件输入停顿 ~1.2s 后，服务端用同一解释器 `ast.parse` 校验，语法错误带行列号红色波浪线（真实模式；mock 模式无服务端自动跳过）。
