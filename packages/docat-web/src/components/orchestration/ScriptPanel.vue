...
const DOCAT_API_JS: DocatCompletionItem[] = [
  ...
  { label: 'poses.get', detail: '按姿态名取坐标', documentation: 'poses.get(姿态名) → 数组；poses.get(姿态名, \';\', digits) → 文本（digits 覆盖小数位数）', insert: 'poses.get(\'${1:姿态名}\')' },
  ...
  { label: 'utils.toString', detail: '数组拼接为文本', documentation: 'utils.toString(数组, 分隔符=\';\', digits) → 文本；浮点默认 6 位小数（通用设置可调），digits 可覆盖', insert: 'utils.toString(${1:数组})' },
...
const DOCAT_API_PY: DocatCompletionItem[] = [
  ...
  { label: 'docat.poses.get', detail: '按姿态名取坐标', documentation: "docat.poses.get(姿态名) → 数组；docat.poses.get(姿态名, sep=';', digits=None) → 文本（digits 覆盖小数位数）", insert: 'docat.poses.get(\'${1:姿态名}\')' },
  ...
  { label: 'docat.utils.to_string', detail: '数组拼接为文本', documentation: "docat.utils.to_string(数组, sep=';', digits=None) → 文本；浮点默认 6 位小数（通用设置可调），digits 可覆盖", insert: 'docat.utils.to_string(${1:数组})' },
