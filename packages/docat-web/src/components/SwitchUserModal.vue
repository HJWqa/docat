    ...
    <div v-if="visible" class="modal-overlay" @click.self="close">
      <div class="modal card">
        ...
        <p class="mt-1" style="color:var(--text-secondary);font-size:13px;">
          Enter the credentials of the account you want to switch to.
        </p>
        <form @submit.prevent="doSwitch" class="modal-form mt-2">
          <div class="field-group">
            <label class="field-label">USERNAME</label>
            <input v-model="username" class="input" placeholder="Target username" autocomplete="username" />
          </div>
          <div class="field-group mt-1">
            <label class="field-label">PASSWORD</label>
            <input v-model="password" class="input" type="password" placeholder="Target password" autocomplete="current-password" />
          </div>
          ...
          <div class="modal-actions mt-2">
            ...
            <button type="submit" class="btn btn-primary flex-1" :disabled="switching || !username || !password">{{ switching ? 'SWITCHING...' : 'SWITCH' }}</button>
          </div>
        </form>
      </div>
    </div>
...
async function doSwitch() {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      error.value = res.error?.message ?? 'Switch failed'
    }
  } catch (err) {
